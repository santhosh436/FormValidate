...
function Validate(){
	if(checkTextEmpty(username,'*username',uname) && checkTextEmpty(email,"email",eml) && checkTextEmpty(password,'*password',pass) && validateEmail(email,eml) && comparepasswords(password,cpassword,cpass)){
		return true;
	}
	else{
		return false;
    ...
    function checkTextEmpty(username,message,uname){
	if(username.value=="")
		{
			uname.innerHTML=message + " is required";
			username.focus();
